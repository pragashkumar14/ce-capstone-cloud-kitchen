...
- **A silent SNS notification failure.** Adding KMS encryption to the SNS alerts topic (as a Prowler-recommended fix) silently broke CloudWatch's ability to publish to it — the alarm correctly fired every time, but zero notifications were ever delivered, across two different email providers, with no obvious error visible anywhere except the CloudWatch alarm's own action history.
...
The account mix-up was resolved by tracing the actual AWS account ID through `aws sts get-caller-identity`, confirming the environment variable was the cause, cleanly destroying everything built in the wrong account, and rebuilding from scratch under the correct one — the Terraform code itself needed zero changes, only the credentials pointing at it. The GitHub rate-limit issue was diagnosed by directly testing an anonymous clone from inside a failing instance and from a fresh local shell to isolate GitHub itself (not the repo configuration) as the cause, then fixed by migrating application deployment to a private, versioned S3 bucket with an automated GitHub Actions upload pipeline — removing the external dependency entirely rather than just waiting for the rate limit to clear. Code bugs were caught by adopting a stricter verification habit: compiling and doing a full file read-through after every edit, rather than trusting a partial diff. The SNS notification failure was diagnosed by checking the CloudWatch alarm's own action history, which revealed the real error message — "CloudWatch Alarms does not have authorization to access the SNS topic encryption key" — directly, rather than continuing to guess at email deliverability issues; the fix was to revert the KMS encryption, since a working alert pipeline mattered more than encryption-at-rest on notification message bodies for this project's timeline. The missing authentication was fixed by adding HTTP Basic Auth backed by Secrets Manager credentials, reusing the existing IAM permission pattern rather than provisioning a new secret.
...
- Real incident diagnosis under time pressure — tracing a failure from a symptom (unhealthy instances, or a missing notification) back to a root cause using logs, direct testing, and process of elimination rather than guessing.
...
- Understanding real AWS service interaction limitations — specifically, that enabling encryption on a resource can silently break another AWS service's ability to interact with it unless permissions are explicitly granted, and that a resource "working" on its own doesn't guarantee every consumer of it still works.

## Key Takeaways

Infrastructure automation doesn't remove the need for careful verification — it makes mistakes propagate faster if you skip it. The habit of checking `aws sts get-caller-identity` before any destructive action, and re-reading a full file after every edit rather than trusting a diff, caught real problems before they became worse. Security testing is most valuable when findings are actually triaged and acted on, not just run once and filed away — the difference between a checklist exercise and a real security posture is whether anything changes as a result. A security fix that silently breaks a different working feature is still a regression, even if it improves the specific finding it targeted — checking downstream effects matters as much as applying the fix itself.

## What Would Be Done Differently

Environment isolation would be set up more deliberately from the start — a dedicated terminal profile or explicit account confirmation step before any `terraform apply`, rather than discovering an account mix-up after infrastructure was already partially built. Deployment would be S3-based from day one rather than GitHub-based, avoiding the rate-limit incident entirely instead of hitting it and then fixing it. Authentication on internal-only routes would be designed in from the first version of those routes, rather than retrofitted after the fact. Any change to a working notification or alerting pipeline would be immediately followed by an end-to-end test (not just a Terraform apply success), rather than assuming a config change that "looks correct" actually preserves existing functionality.

## Future Improvements Planned

Beyond the capstone's scope: Multi-AZ RDS with automated failover, a customer-managed KMS key for SNS encryption with a correctly scoped key policy granting CloudWatch the permissions it needs (rather than leaving the topic unencrypted, as was done under time pressure here), Secrets Manager automatic rotation, a WAF layer in front of the ALB, and role-based access for kitchen staff instead of a single shared admin credential.
